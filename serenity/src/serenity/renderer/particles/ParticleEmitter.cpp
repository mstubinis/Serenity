...
ParticleEmitter::ParticleEmitter(Scene& scene_, ParticleEmissionPropertiesHandle properties, float lifetime, Entity parent)
    : Entity{ scene_ }
{
    addComponent<ComponentTransform>();

    //internal_add_debugging_visual(*this);
    init(scene_, properties, lifetime, parent);
}
void ParticleEmitter::init(Scene& scene, ParticleEmissionPropertiesHandle properties, float lifetime, Entity parent) {
    ...
    auto prop = Engine::Resources::getParticleEmissionProperties(m_Properties);
    assert(prop != nullptr);

    m_SpawningTimer = prop->m_SpawnRate - 0.01f;
...
ParticleEmitter::ParticleEmitter(ParticleEmitter&& other) noexcept 
    ...
    , m_Properties   { std::exchange(other.m_Properties, ParticleEmissionPropertiesHandle{}) }
...
void ParticleEmitter::setProperties(ParticleEmissionPropertiesHandle properties) {
...
void ParticleEmitter::update(size_t index, const float dt, Engine::priv::ParticleSystem& particleSystem, bool multi_threaded) {
    ...
    if (m_Active) {
        m_Timer           += dt;
        m_SpawningTimer   += dt;
        if (m_Properties && m_SpawningTimer > prop->m_SpawnRate) {
            std::scoped_lock lock{ particleSystem.m_Mutex };
