...
void Particle::init(const glm::vec3& emitterPosition, const glm::quat& emitterRotation, ParticleEmitter& emitter) noexcept {
    ...
    auto& prop = *Engine::Resources::getParticleEmissionProperties(emitter.m_Properties);
    m_Material                    = &const_cast<Material&>(prop.getParticleMaterialRandom());

    auto& emitterTransform        = *emitter.getComponent<ComponentTransform>();
    auto emitterScale             = emitterTransform.getScale();

    m_Velocity                    = prop.m_InitialVelocityFunctor() * emitterScale;
    ...
    */
    ...
    m_Scale           = prop.m_InitialScaleFunctor() * Engine::Math::Max(emitterScale.x, emitterScale.y, emitterScale.z);
    m_AngularVelocity = prop.m_InitialAngularVelocityFunctor();
    m_Color           = prop.m_ColorFunctor(0.0f, prop.getLifetime(), m_Timer);
}
...
float Particle::lifetime() const noexcept {
    auto& prop = *Engine::Resources::getParticleEmissionProperties(m_EmitterSource->m_Properties);
    return prop.m_Lifetime;
}
