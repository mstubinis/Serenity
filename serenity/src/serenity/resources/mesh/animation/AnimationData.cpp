...
void AnimationData::ComputeTransforms(float TimeInSeconds, std::vector<glm::mat4>& Xforms, MeshSkeleton& skeleton, MeshNodeData& nodeData) {
    ...
    for (uint32_t i = 1; i < nodeData.m_Nodes.size(); ++i) {
        ...
        if (!channel.empty()) {
            const glm::vec3 s   = CalcInterpolatedScaling(AnimationTime, channel.ScalingKeys, channel.CurrentKeyframes[2]);
            const glm::quat q   = CalcInterpolatedRotation(AnimationTime, channel.RotationKeys, channel.CurrentKeyframes[1]);
            const glm::vec3 t   = CalcInterpolatedPosition(AnimationTime, channel.PositionKeys, channel.CurrentKeyframes[0]);
