...
void MeshCPUData::internal_calculate_radius() {
    ...
    const auto points = m_VertexData->getPositions(); //expensive
...
void Engine::priv::PublicMesh::FinalizeVertexData(MeshCPUData& cpuData, MeshImportedData& data) {
    ...
    if (cpuData.m_Threshold == 0.0f) {
        ...
        for (const auto& tempNormal : data.m_Normals) {
            normals[0].emplace_back(Engine::Compression::pack3NormalsInto32Int(tempNormal));
        }
        for (const auto& tempBinormal : data.m_Binormals) {
            normals[1].emplace_back(Engine::Compression::pack3NormalsInto32Int(tempBinormal));
        }
        for (const auto& tempTangent : data.m_Tangents) {
            normals[2].emplace_back(Engine::Compression::pack3NormalsInto32Int(tempTangent));
        }
        ...
        for (const auto& tempNormal : temp_normals) {
            normals[0].emplace_back(Engine::Compression::pack3NormalsInto32Int(tempNormal));
        }
        for (const auto& tempBinormal : temp_binormals) {
            normals[1].emplace_back(Engine::Compression::pack3NormalsInto32Int(tempBinormal));
        }
        for (const auto& tempTangent : temp_tangents) {
            normals[2].emplace_back(Engine::Compression::pack3NormalsInto32Int(tempTangent));
        }
...
void Mesh::unload(bool dispatchEventUnloaded) {
    if(isLoaded()){
...
void Mesh::sortTriangles(const Camera& camera,  const ModelInstance& instance, const glm::mat4& bodyModelMatrix, SortingMode sortMode) {
        ...
        if (!triangles.empty()) {
            ...
            const glm::mat4 worldModelMatrix = instance.getModelMatrix() * bodyModelMatrix;
            auto lambda_sorter = [&camPos, sortMode, &worldModelMatrix](const auto& lhs, const auto& rhs) {
                glm::mat4 model1 = worldModelMatrix;
                glm::mat4 model2 = worldModelMatrix;

                const auto& lhsMidpt = lhs.getMidpoint();
                const auto& rhsMidpt = rhs.getMidpoint();

                model1[3] = model1[0] * lhsMidpt[0] + model1[1] * lhsMidpt[1] + model1[2] * lhsMidpt[2] + model1[3];
                model2[3] = model2[0] * rhsMidpt[0] + model2[1] * rhsMidpt[1] + model2[2] * rhsMidpt[2] + model2[3];

                const auto m1Dist = glm::distance2(camPos, glm::vec3{ model1[3] });
                const auto m2Dist = glm::distance2(camPos, glm::vec3{ model2[3] });

                return (sortMode == SortingMode::FrontToBack) ? (m1Dist < m2Dist) : (m1Dist > m2Dist);
            };
            ...
            for (size_t i = 0; i < triangles.size(); ++i) {
                for (size_t j = 0; j < 3; ++j) {
                    newIndices.emplace_back(triangles[i].getIndex(j));
                }
            }
