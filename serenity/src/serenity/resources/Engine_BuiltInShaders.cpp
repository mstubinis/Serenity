...
#pragma region ParticleVertex
    ...
    #define ENGINE_PARTICLES_HALF_SIZE_SIZE_NUMBER 16
#else
    #define ENGINE_PARTICLES_HALF_SIZE_SIZE_NUMBER 32
#endif
...
Engine::priv::EShaders::particle_vertex =
    ...
    "const vec3 ParticlePosition = aParticlePositionAndScaleX.xyz;\n"
    "const float ScaleX = aParticlePositionAndScaleX.w;\n"
    "const float ScaleY = aParticleScaleYAndRotation.x;\n"

    "void main() {\n"
    "    const float sin_ = sin(aParticleScaleYAndRotation.y);\n"
    "    const float cos_ = cos(aParticleScaleYAndRotation.y);\n"

    "    const float xPrime = aVertexPosition.x * cos_ - aVertexPosition.y * sin_;\n"
    "    const float yPrime = aVertexPosition.x * sin_ + aVertexPosition.y * cos_;\n"

    "    const vec3 CameraRight = vec3(CameraView[0][0], CameraView[1][0], CameraView[2][0]);\n"
    "    const vec3 CameraUp = vec3(CameraView[0][1], CameraView[1][1], CameraView[2][1]);\n"
    "    const vec3 VertexWorldSpace = (ParticlePosition) + CameraRight * xPrime * ScaleX + CameraUp * yPrime * ScaleY;\n"

    "    const vec4 worldPos = vec4(VertexWorldSpace, 1.0);\n"
    "    gl_Position = CameraViewProj * worldPos;\n"
    "    WorldPosition = worldPos.xyz;\n"
    "    UV = aUV;\n"
    "    MaterialIndex = aParticleMaterialIndexAndColorPacked.x;\n"
    "    ParticleColor = Unpack" + std::to_string(ENGINE_PARTICLES_HALF_SIZE_SIZE_NUMBER) + "BitUIntTo4ColorFloats(aParticleMaterialIndexAndColorPacked.y);\n"
    "}";
#pragma endregion
