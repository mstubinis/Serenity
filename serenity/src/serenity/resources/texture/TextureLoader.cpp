...
void TextureLoader::LoadGPU(Handle textureHandle, bool dispatchEventLoaded) {
    std::scoped_lock lock{ Engine::Resources::getMutex() };
...
void TextureLoader::Load(Texture& texture, bool dispatchEventLoaded) {
    if (!texture.isLoaded()) {
        TextureLoader::LoadCPU(texture.m_CPUData, Handle{});
        TextureLoader::LoadGPU(texture, dispatchEventLoaded);
    }
}
