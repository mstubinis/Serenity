...
void Engine::priv::GLDebugDrawer::init() {
    ...
    m_VBO.generate();
    m_VBO.bind();
    m_VBO.setData(0, nullptr, BufferDataDrawType::Dynamic);
...
void Engine::priv::GLDebugDrawer::bindDataToGPU() {
    m_VBO.bind();
    m_VBO.specifyVertexAttribute(0, 3, GL_FLOAT, false, sizeof(LineVertex), 0);
    m_VBO.specifyVertexAttribute(1, 3, GL_FLOAT, false, sizeof(LineVertex), offsetof(LineVertex, color));
}
void Engine::priv::GLDebugDrawer::render() {
    if (m_VAO) {
        m_VAO.bindVAO();
        glDrawArrays(GL_LINES, 0, GLsizei(m_PerFrameCount));
        m_VAO.unbindVAO();
    } else {
        bindDataToGPU();
        glDrawArrays(GL_LINES, 0, GLsizei(m_PerFrameCount));
...
void Engine::priv::GLDebugDrawer::drawAccumulatedLines() {
    if (m_PerFrameCount > 0) {
        m_VBO.bind();
        m_VBO.setData(sizeof(LineVertex) * m_PerFrameCount, m_LineVertices.data(), BufferDataDrawType::Dynamic);
        render();
    }
}
