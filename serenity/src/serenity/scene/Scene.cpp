...
Scene::Scene(uint32_t id, std::string_view name, const SceneOptions& options) 
    ...
    , m_ParticleSystemNew{ NEW Engine::priv::ParticleSystemNew(options.maxAmountOfParticleEmitters, options.maxAmountOfParticles) }
    , m_ID{ id }
{
    m_ECS.init(options);
...
Scene::~Scene() {
    ...
    Engine::getRenderer().m_Pipeline->deleteAllShadowCasters(*this);
    SAFE_DELETE(m_ParticleSystemNew);
...
Engine::view_ptr<ParticleEmitter> Scene::addParticleEmitter(ParticleEmissionPropertiesHandle properties, float lifetime, Entity parent) {
    return m_ParticleSystem.add_emitter(properties, *this, lifetime, parent);
}
ParticleEmitterHandle Scene::addParticleEmitterNew(ParticleEmissionPropertiesHandle handle, float lifetime) {
    return (handle) ? m_ParticleSystemNew->add_emitter(handle, *this, lifetime) : ParticleEmitterHandle{};
}
...
//new
Engine::priv::ParticleEmitterContainer<ParticleEmitter, ParticleEmitterHandle>& Engine::priv::PublicScene::GetParticleEmitters(Scene& scene) {
    return scene.m_ParticleSystemNew->m_ParticleEmitterContainer;
}
...
void Engine::priv::PublicScene::UpdateParticles(Scene& scene, const float dt) {
    scene.m_ParticleSystem.update(dt, *scene.getActiveCamera());
    scene.m_ParticleSystemNew->update(dt);
}
...
void Engine::priv::PublicScene::RenderParticles(RenderModule& renderer, Scene& scene, Viewport& viewport, Camera& camera) {
    scene.m_ParticleSystem.render(viewport, camera, renderer);
    scene.m_ParticleSystemNew->render(viewport, camera, renderer);
}
