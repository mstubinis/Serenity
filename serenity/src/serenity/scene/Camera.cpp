...
namespace {
    CameraLogicFunctionPtr CAMERA_DEFAULT_MOUSELOOK_FUNCTION = [](const CameraLogicComponent* component, const float dt) {
        ...
        auto mouse             = Engine::getMouseDifference();

        auto dtFactor          = glm::pow(float(ENGINE_FIXED_TIMESTEP_VALUE) / dt, 655.0f);
        //std::cout << dtFactor << '\n';

        const auto rotSpeedX   = ((mouse.x) * dtFactor) * 0.006f;
        const auto rotSpeedY   = (-(mouse.y) * dtFactor) * 0.006f;
