...
#include <serenity/editor/imgui/imconfig.h>
#endif
...
namespace ImGui {
    template<class T>
    void InputNumber(const char* id, T* val, T step = static_cast<T>(0.0), T stepFast = static_cast<T>(0.0), const char* format = "%.6f", ImGuiInputTextFlags flags = 0) {
        if constexpr (std::is_same<T, double>()) {
            ImGui::InputDouble(id, val, step, stepFast, format, flags);
        }else if (std::is_same<T, float>()) {
            ImGui::InputFloat(id, val, step, stepFast, format, flags);
        }else if (std::is_same<T, int>()) {
            ImGui::InputInt(id, val, step, stepFast, flags);
        }
    }

    template<class T>
    void InputNumber2(const char* id, T* val, T step = static_cast<T>(0.0), T stepFast = static_cast<T>(0.0), const char* format = "%.6f", ImGuiInputTextFlags flags = 0) {
        if constexpr (std::is_same<T, double>()) {
            ImGui::InputDouble(id, val, step, stepFast, format, flags);
        }
        else if (std::is_same<T, float>()) {
...
#include <serenity/editor/core/imgui_user.h>
#endif
