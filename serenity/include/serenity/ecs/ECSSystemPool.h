    ...
    class ECSSystemPool final {
        ...
        private:
            struct OrderEntry final {
                ...
                uint32_t typeID;
                OrderEntry() = default;
                OrderEntry(int order_, uint32_t typeID_)
                    : order { order_ }
                    , typeID { typeID_ }
                {}
        ...
        public:
            ...
            SYSTEM* registerSystem(Engine::priv::ECS& ecs, ARG_TUPLE&& argTuple) {
                ...
                Engine::priv::detail::construct_system<SYSTEM>(m_Systems, ecs, std::forward<ARG_TUPLE>(argTuple));
            ...
            template<class SYSTEM, class ARG_TUPLE, class ... COMPONENTS>
            SYSTEM* registerSystemOrdered(int order, Engine::priv::ECS& ecs, ARG_TUPLE&& argTuple) {
                SYSTEM* createdSystem = nullptr;
                if (SYSTEM::TYPE_ID == std::numeric_limits<uint32_t>().max()) {
                    SYSTEM::TYPE_ID = m_RegisteredSystems++;
                }
            ...
            //(SystemBaseClass*, int&) should be the function arguments
            template<class FUNC>
            void forEachOrdered(FUNC&& func) {
                for (size_t i = 0; i < m_Order.size(); ++i) {
                    func(m_Systems[m_Order[i].typeID], m_Order[i].order);
                }
            }
