...
class Terrain : public Observer, public Entity {
    ...
    public:
        Terrain(const std::string& name, sf::Image& heightmapImage, Handle& material, uint32_t pointsPerPixel = 2, float heightScale = 1.0f, int blurIterations = 4, bool useDiamondSubdivisions = false, Scene* = nullptr);
        virtual ~Terrain();
