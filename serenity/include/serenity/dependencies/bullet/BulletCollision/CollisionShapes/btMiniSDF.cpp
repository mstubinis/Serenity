...
bool btMiniSDF::load(const char* data, int size)
{
...

	{
		...
		for (int i = 0; i < (int)n_cell_maps0; i++)
		{
			...
			for (int j = 0; j < (int)n_cell_maps1; j++)
			{
...
btMiniSDF::subdomain(btMultiIndex const& ijk) const
{
	btAssert(m_isValid);
	btVector3 tmp;
	tmp.m_floats[0] = m_cell_size[0] * btScalar((double)ijk.ijk[0]);
	tmp.m_floats[1] = m_cell_size[1] * btScalar((double)ijk.ijk[1]);
	tmp.m_floats[2] = m_cell_size[2] * btScalar((double)ijk.ijk[2]);
							...
							btVector3* gradient) const
{
	...
	for (unsigned int j = 0u; j < 32u; ++j)
	{
		...
		phi += c * N[j];
		(*gradient)[0] += btScalar(c) * dN(j, 0);
		(*gradient)[1] += btScalar(c) * dN(j, 1);
		(*gradient)[2] += btScalar(c) * dN(j, 2);
	}
	(*gradient) *= c0;
	dist = phi;
	return true;
