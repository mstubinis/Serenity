...
struct btDbvntNode
{
    ...
    DBVT_INLINE bool isleaf() const { return (childs[1] == 0); }
    DBVT_INLINE bool isinternal() const { return (!isleaf()); }
	btDbvntNode* childs[2] = {nullptr, nullptr};
    void* data = nullptr;
...
struct btDbvt
{
	...
	struct sStkNN
	{
		const btDbvtNode* a = nullptr;
		const btDbvtNode* b = nullptr;
		sStkNN() {}
		sStkNN(const btDbvtNode* na, const btDbvtNode* nb) : a(na), b(nb) {}
	};
	struct sStkNP
	{
		const btDbvtNode* node = nullptr;
		int mask = 0;
	...
	{
		const btDbvtNode* node = nullptr;
		btDbvtNode* parent = nullptr;
		sStkCLN(const btDbvtNode* n, btDbvtNode* p) : node(n), parent(p) {}
	};
    
    struct sStknNN
    {
        const btDbvntNode* a = nullptr;
        const btDbvntNode* b = nullptr;
        sStknNN() {}
