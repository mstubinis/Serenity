...
std::string Engine::Shaders::Detail::ShadersManagement::deferred_frag = 
	...
	"varying float logz_f;\n"
    "\n"
	"const float decode_x = 1.0/255.0;\n"
	"const float decode_y = 1.0/65025.0;\n"
	"const float decode_z = 1.0/16581375.0;\n"
	"float EncodeFloatRGBA( vec4 rgba ) {\n"
	"	return dot( rgba, vec4(1.0, decode_x, decode_y, decode_z) );\n"
	"}\n"
	...
	"		smoothness = BaseSmoothness * texture2D(SmoothnessTexture, UV).r;\n"
	"	}\n"
	"	if(ThirdConditionals.y > 0.5){\n"
	"		metalness = BaseMetalness * texture2D(MetalnessTexture, UV).r;\n"
	"	}\n"
	"	if(ThirdConditionals.x > 0.5){\n"
	"		ao = BaseAO * texture2D(AOTexture, UV).r;\n"
	"	}\n"
	"\n"
	"	gl_FragData[1].a = EncodeFloatRGBA(vec4(matID,smoothness,ao,metalness));\n"
...
std::string Engine::Shaders::Detail::ShadersManagement::lighting_frag = 
	...
	"	enc = fract(enc);\n"
	"	enc -= enc.yzww * vec4(decode_x,decode_x,decode_x,0.0);\n"
	"	return enc;\n"
	"}\n"
	"vec3 reconstruct_world_pos(vec2 _uv){\n"
	"	float log_depth = texture2D(gDepthMap, _uv).r;\n"
	...
	"\n"
	"   vec4 stuff = DecodeFloatRGBA(texture2D(gNormalMap,uv).a);\n"
    "\n"
	"	vec3 SpecularFactor = vec3(0.0);\n"
    "\n"
	"	float kPi = 3.1415926535898;\n"
	"	highp int index = int(stuff.x);\n"
	"	float metalness = stuff.a;\n"
