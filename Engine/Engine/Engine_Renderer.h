...
#pragma region GeneralInlineSendUniforms
        ...
        inline void sendUniformMatrix2x4(const char* l,glm::mat2x4& m){ glUniformMatrix2x4fv(getUniformLocationUnsafe(l),1,0,glm::value_ptr(m)); }
        inline void sendUniformMatrix2x4v(const char* l,std::vector<glm::mat2x4>& m,uint count){ std::vector<float> d;for(auto ma:m){float* m = glm::value_ptr(ma);for(uint i = 0; i < 8; i++){d.push_back(m[i]);}}glUniformMatrix2x4fv(getUniformLocationUnsafe(l),count,0,&d[0]); }
        ...
        inline void sendUniformMatrix4v(const char* l,std::vector<glm::dmat4>& m,uint count){ std::vector<glm::highp_float> d;for(auto ma:m){glm::highp_float* m = glm::value_ptr(ma);for(uint i = 0; i < 16; i++){d.push_back(m[i]);}}glUniformMatrix4dv(getUniformLocationUnsafe(l),count,0,&d[0]); }
...
#pragma region GeneralInlineSendUniformSafes
...
#pragma endregion
#pragma region GeneralInlineSendUniformForces
