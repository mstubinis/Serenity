...
namespace Engine{
    ...
    namespace Renderer{
        ...
        inline void sendUniformMatrix2v(const char* l, const std::vector<glm::mat2>& m,uint count){ std::vector<float> d;for(auto& ma:m){const float* m = glm::value_ptr(ma);for(uint i = 0; i < 4; ++i){d.push_back(m[i]);}}glUniformMatrix2fv(getUniformLocUnsafe(l),count,0,&d[0]); }
        ...
        inline void sendUniformMatrix2v(const char* l, const std::vector<glm::dmat2>& m,uint count){ std::vector<double> d;for(auto& ma:m){ const double* m = glm::value_ptr(ma);for(uint i = 0; i < 4; ++i){d.push_back(m[i]);}}glUniformMatrix2dv(getUniformLocUnsafe(l),count,0,&d[0]); }
        ...
        inline void sendUniformMatrix2vSafe(const char* l, const std::vector<glm::mat2>& m, uint count){ const auto& o=getUniformLoc(l);if(o==-1)return;std::vector<float> d;for(auto& ma:m){ const float* m = glm::value_ptr(ma);for(uint i = 0; i < 4; ++i){d.push_back(m[i]);}}glUniformMatrix2fv(o,count,0,&d[0]); }
        ...
        inline void sendUniformMatrix2vSafe(const char* l, const std::vector<glm::dmat2>& m, uint count){ const auto& o=getUniformLoc(l);if(o==-1)return;std::vector<double> d;for(auto& ma:m){ const double* m = glm::value_ptr(ma);for(uint i = 0; i < 4; ++i){d.push_back(m[i]);}}glUniformMatrix2dv(o,count,0,&d[0]); }
        inline void sendUniformMatrix2Force(const char* l, const glm::mat2& m){ glUniformMatrix2fv(getUniformLoc(l),1,0,glm::value_ptr(m)); }
        inline void sendUniformMatrix2vForce(const char* l, const std::vector<glm::mat2>& m,uint count){ std::vector<float> d;for(auto& ma:m){ const float* m = glm::value_ptr(ma);for(uint i = 0; i < 4; ++i){d.push_back(m[i]);}}glUniformMatrix2fv(getUniformLoc(l),count,0,&d[0]); }
        ...
        inline void sendUniformMatrix2vForce(const char* l, const std::vector<glm::dmat2>& m,uint count){ std::vector<double> d;for(auto& ma:m){ const double* m = glm::value_ptr(ma);for(uint i = 0; i < 4; ++i){d.push_back(m[i]);}}glUniformMatrix2dv(getUniformLoc(l),count,0,&d[0]); }
        ...
        inline void sendUniformMatrix3v(const char* l, const std::vector<glm::mat3>& m,uint count){ std::vector<float> d;for(auto& ma:m){ const float* m = glm::value_ptr(ma);for(uint i = 0; i < 9; ++i){d.push_back(m[i]);}}glUniformMatrix3fv(getUniformLocUnsafe(l),count,0,&d[0]); }
        ...
        inline void sendUniformMatrix3v(const char* l, const std::vector<glm::dmat3>& m,uint count){ std::vector<double> d;for(auto& ma:m){ const double* m = glm::value_ptr(ma);for(uint i = 0; i < 9; ++i){d.push_back(m[i]);}}glUniformMatrix3dv(getUniformLocUnsafe(l),count,0,&d[0]); }
        ...
        inline void sendUniformMatrix3vSafe(const char* l, const std::vector<glm::mat3>& m, uint count){ const auto& o=getUniformLoc(l);if(o==-1)return;std::vector<float> d;for(auto& ma:m){ const float* m = glm::value_ptr(ma);for(uint i = 0; i < 9; ++i){d.push_back(m[i]);}}glUniformMatrix3fv(o,count,0,&d[0]); }
        ...
        inline void sendUniformMatrix3vSafe(const char* l, const std::vector<glm::dmat3>& m, uint count){ const auto& o=getUniformLoc(l);if(o==-1)return;std::vector<double> d;for(auto& ma:m){ const double* m = glm::value_ptr(ma);for(uint i = 0; i < 9; ++i){d.push_back(m[i]);}}glUniformMatrix3dv(o,count,0,&d[0]); }
        inline void sendUniformMatrix3Force(const char* l, const glm::mat3& m){ glUniformMatrix3fv(getUniformLoc(l),1,0,glm::value_ptr(m)); }
        inline void sendUniformMatrix3vForce(const char* l, const std::vector<glm::mat3>& m,uint count){ std::vector<float> d;for(auto& ma:m){ const float* m = glm::value_ptr(ma);for(uint i = 0; i < 9; ++i){d.push_back(m[i]);}}glUniformMatrix3fv(getUniformLoc(l),count,0,&d[0]); }
        ...
        inline void sendUniformMatrix3vForce(const char* l, const std::vector<glm::dmat3>& m,uint count){ std::vector<double> d;for(auto& ma:m){ const double* m = glm::value_ptr(ma);for(uint i = 0; i < 9; ++i){d.push_back(m[i]);}}glUniformMatrix3dv(getUniformLoc(l),count,0,&d[0]); }
        ...
        inline void sendUniformMatrix4v(const char* l, const std::vector<glm::mat4>& m,uint count){ std::vector<float> d;for(auto& ma:m){ const float* m = glm::value_ptr(ma);for(uint i = 0; i < 16; ++i){d.push_back(m[i]);}}glUniformMatrix4fv(getUniformLocUnsafe(l),count,0,&d[0]); }
        ...
        inline void sendUniformMatrix4v(const char* l, const std::vector<glm::dmat4>& m,uint count){ std::vector<double> d;for(auto& ma:m){ const double* m = glm::value_ptr(ma);for(uint i = 0; i < 16; ++i){d.push_back(m[i]);}}glUniformMatrix4dv(getUniformLocUnsafe(l),count,0,&d[0]); }
        ...
        inline void sendUniformMatrix4vSafe(const char* l, const std::vector<glm::mat4>& m, uint count){ const auto& o=getUniformLoc(l);if(o==-1)return;std::vector<float> d;for(auto& ma:m){ const float* m = glm::value_ptr(ma);for(uint i = 0; i < 16; ++i){d.push_back(m[i]);}}glUniformMatrix4fv(o,count,0,&d[0]); }
        ...
        inline void sendUniformMatrix4vSafe(const char* l, const std::vector<glm::dmat4>& m, uint count){ const auto& o=getUniformLoc(l);if(o==-1)return;std::vector<double> d;for(auto& ma:m){ const double* m = glm::value_ptr(ma);for(uint i = 0; i < 16; ++i){d.push_back(m[i]);}}glUniformMatrix4dv(o,count,0,&d[0]); }
        inline void sendUniformMatrix4Force(const char* l, const glm::mat4& m){ glUniformMatrix4fv(getUniformLoc(l),1,0,glm::value_ptr(m)); }
        inline void sendUniformMatrix4vForce(const char* l, const std::vector<glm::mat4>& m,uint count){ std::vector<float> d;for(auto& ma:m){ const float* m = glm::value_ptr(ma);for(uint i = 0; i < 16; ++i){d.push_back(m[i]);}}glUniformMatrix4fv(getUniformLoc(l),count,0,&d[0]); }
        ...
        inline void sendUniformMatrix4vForce(const char* l, const std::vector<glm::dmat4>& m,uint count){ std::vector<double> d;for(auto& ma:m){ const double* m = glm::value_ptr(ma);for(uint i = 0; i < 16; ++i){d.push_back(m[i]);}}glUniformMatrix4dv(getUniformLoc(l),count,0,&d[0]); }
