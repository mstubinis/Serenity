...
std::string Engine::Shaders::Detail::ShadersManagement::final_frag = 
    ...
    "    if(HasSSAO == 1){ \n"
	"       gl_FragColor = vec4(vec3(stuff.x),1.0);\n"
	"    }else{\n"
	"       gl_FragColor = vec4(vec3(stuff.y),1.0);\n"
	"    }\n"
    "}";
...
std::string Engine::Shaders::Detail::ShadersManagement::lighting_frag = 
    ...
    "    highp int index = 0;\n"
	"    vec2 stuff = Unpack16BitFloatInto2Floats(texture2D(gNormalMap,uv).a);\n"
	"    float smoothness = stuff.x;\n"
	"    float metalness = stuff.y;\n"
    "\n"
