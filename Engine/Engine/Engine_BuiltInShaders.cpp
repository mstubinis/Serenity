...
epriv::EShaders::forward_frag +=
    ...
    "        gl_FragData[1].rg = EncodeOctahedron(CalcBumpedNormalCompressed(uv,NormalTexture));\n"
    "    }\n"
...
epriv::EShaders::ssao_frag +=
    ...
    "        float brightness = log(dot(lighting, vec3(0.2126, 0.7152, 0.0722)));\n"
    "        float scale = max(Glow, brightness);\n"
    "        gl_FragColor.rgb = (lighting * scale * scale * scale );\n"
    "    }\n"
    "    else{\n"
