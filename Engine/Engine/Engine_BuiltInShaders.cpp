...
Shaders::Detail::ShadersManagement::smaa_vertex_1 = Shaders::Detail::ShadersManagement::version + Shaders::Detail::ShadersManagement::smaa_common +
    ...
    "    _offset[0] = mad(SMAA_PIXEL_SIZE.xyxy,vec4(-1.0, 0.0, 0.0, -1.0),uv.xyxy);\n"
    "    _offset[1] = mad(SMAA_PIXEL_SIZE.xyxy,vec4( 1.0, 0.0, 0.0,  1.0),uv.xyxy);\n"
    "    _offset[2] = mad(SMAA_PIXEL_SIZE.xyxy,vec4(-2.0, 0.0, 0.0, -2.0),uv.xyxy);\n"
...
Shaders::Detail::ShadersManagement::smaa_vertex_2 = Shaders::Detail::ShadersManagement::version + Shaders::Detail::ShadersManagement::smaa_common +
    ...
    "    _offset[0] = mad(SMAA_PIXEL_SIZE.xyxy,vec4(-0.25,-0.125, 1.25,-0.125),uv.xyxy);\n"
    "    _offset[1] = mad(SMAA_PIXEL_SIZE.xyxy,vec4(-0.125,-0.25,-0.125,1.25),uv.xyxy);\n"
    "    _offset[2] = mad(SMAA_PIXEL_SIZE.xxyy,vec4(-2.0, 2.0, -2.0, 2.0) * float(SMAA_MAX_SEARCH_STEPS),vec4(_offset[0].xz, _offset[1].yw));\n"
    "    gl_Position = MVP * gl_Vertex;\n"
...
Shaders::Detail::ShadersManagement::smaa_frag_2 = Shaders::Detail::ShadersManagement::version + Shaders::Detail::ShadersManagement::smaa_common +
    ...
    "    if (d.x + d.y > 2.0) {\n"
    "        vec4 coords = mad(vec4(-d.x + 0.25, d.x, d.y, -d.y - 0.25), _SMAA_PIXEL_SIZE.xyxy, texcoord.xyxy);\n"
    "        vec4 c;\n"
    "        c.xy = texture2DLod(edgesTex, coords.xy + vec2(-1.0,0.0), 0.0).rg;\n"
    "        c.zw = texture2DLod(edgesTex, coords.zw + vec2(1.0,0.0), 0.0).rg;\n"
    ...
    "    if (texture2DLod(edgesTex, texcoord + vec2(1.0, 0.0), 0.0).r > 0.0) {\n"
    "        d.yw = SMAASearchDiag2(edgesTex, texcoord, vec2(1.0, 1.0), end);\n"
    "        d.y += float(end.y > 0.9);\n"
    "    } else\n"
    "        d.yw = vec2(0.0, 0.0);\n"
    "    if (d.x + d.y > 2.0) {\n"
    "        vec4 coords = mad(vec4(-d.x, -d.x, d.y, d.y), _SMAA_PIXEL_SIZE.xyxy, texcoord.xyxy);\n"
    "        vec4 c;\n"
    "        c.x  = texture2DLod(edgesTex, coords.xy + vec2(-1.0,  0.0), 0.0).g;\n"
    "        c.y  = texture2DLod(edgesTex, coords.xy + vec2( 0.0, -1.0), 0.0).r;\n"
    "        c.zw = texture2DLod(edgesTex, coords.zw + vec2( 1.0,  0.0), 0.0).gr;\n"
    "        vec2 cc = mad(vec2(2.0, 2.0), c.xz, c.yw);\n"
    "        SMAAMovc(bvec2(step(0.9, d.zw)), cc, vec2(0.0, 0.0));\n"
    ...
    "    return (texture2DLod(searchTex, mad(scale, e, bias), 0.0)).r;\n"
    "}\n"
    ...
    "        e = texture2DLod(edgesTex, texcoord,0.0).rg;\n"
    "        texcoord = mad(-vec2(0.0, 2.0), _SMAA_PIXEL_SIZE.xy, texcoord);\n"
    "    }\n"
    "    float offset = mad(-(255.0 / 127.0), SMAASearchLength(searchTex, e.gr, 0.0), 3.25);\n"
    "    return mad(_SMAA_PIXEL_SIZE.y, offset, texcoord.y);\n"
    "}\n"
    ...
    "        factor.x -= rounding.x * texture2DLod(edgesTex, texcoord.xy + vec2(0.0,  1.0), 0.0).r;\n"
    "        factor.x -= rounding.y * texture2DLod(edgesTex, texcoord.zw + vec2(1.0,  1.0), 0.0).r;\n"
    "        factor.y -= rounding.x * texture2DLod(edgesTex, texcoord.xy + vec2(0.0, -2.0), 0.0).r;\n"
    "        factor.y -= rounding.y * texture2DLod(edgesTex, texcoord.zw + vec2(1.0, -2.0), 0.0).r;\n"
    ...
    "        rounding /= leftRight.x + leftRight.y;\n"
    "        vec2 factor = vec2(1.0, 1.0);\n"
    "        factor.x -= rounding.x * texture2DLod(edgesTex, texcoord.xy + vec2( 1.0, 0.0), 0.0).g;\n"
    "        factor.x -= rounding.y * texture2DLod(edgesTex, texcoord.zw + vec2( 1.0, 1.0), 0.0).g;\n"
    "        factor.y -= rounding.x * texture2DLod(edgesTex, texcoord.xy + vec2(-2.0, 0.0), 0.0).g;\n"
    "        factor.y -= rounding.y * texture2DLod(edgesTex, texcoord.zw + vec2(-2.0, 1.0), 0.0).g;\n"
    "        weights *= saturate(factor);\n"
    "    }\n"
    "}\n"
    ...
    "        vec2 sqrt_d = sqrt(d);\n"
    "        float e2 = texture2DLod(edgesTex, coords.zy + vec2(1.0,0.0), 0.0).r;\n"
    ...
    "        vec2 sqrt_d = sqrt(d);\n"
    "        float e2 = texture2DLod(edgesTex, coords.xz + vec2(0.0, 1.0), 0.0).g;\n"
...
Shaders::Detail::ShadersManagement::smaa_vertex_3 = Shaders::Detail::ShadersManagement::version + Shaders::Detail::ShadersManagement::smaa_common +
    ...
    "    mat4 MVP = VP * Model;\n"
    "    uv = gl_MultiTexCoord0.xy;\n"
    "    _offset = mad(SMAA_PIXEL_SIZE.xyxy,vec4(1.0,0.0,0.0,1.0),uv.xyxy);\n"
    "    gl_Position = MVP * gl_Vertex;\n"
    "    _SMAA_PIXEL_SIZE = SMAA_PIXEL_SIZE;\n"
    "}\n"
    "\n";
