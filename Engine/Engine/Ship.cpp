	...
	if(m_Ship->IsPlayer()){
		if(!m_Ship->IsWarping()){
			if(Keyboard::isKeyDown("w")){
				m_Ship->applyForce(0,0,-1*(m_Ship->getMass()*3));
			}
			if(Keyboard::isKeyDown("s")){
				m_Ship->applyForce(0,0,1*(m_Ship->getMass()*3));
			}
			if(Keyboard::isKeyDown("a")){
				m_Ship->applyForce(-1*(m_Ship->getMass()*3),0,0);
			}
			if(Keyboard::isKeyDown("d")){
				m_Ship->applyForce(1*(m_Ship->getMass()*3),0,0);
			}
			if(Keyboard::isKeyDown("f")){
				m_Ship->applyForce(0,-1*(m_Ship->getMass()*3),0);
			}
...
void ShipSystemPitchThrusters::update(float dt){
	if(m_Ship->IsPlayer()){
		if(m_Ship->getPlayerCamera()->getState() != CAMERA_STATE_ORBIT){
			m_Ship->applyTorque(-Mouse::getMouseDifference().y*0.002f*(1/(m_Ship->getMass()*3)),0,0);
		}
	}
	ShipSystem::update(dt);
}
...
void ShipSystemYawThrusters::update(float dt){
	if(m_Ship->IsPlayer()){
		if(m_Ship->getPlayerCamera()->getState() != CAMERA_STATE_ORBIT){
			m_Ship->applyTorque(0,-Mouse::getMouseDifference().x*0.002f*(1/(m_Ship->getMass()*3)),0);
		}
	}
	ShipSystem::update(dt);
}
...
void ShipSystemRollThrusters::update(float dt){
	if(m_Ship->IsPlayer()){
		if(Keyboard::isKeyDown("q")){
			m_Ship->applyTorque(0,0,1*1/(m_Ship->getMass()));
		}
...
Ship::Ship(std::string mesh, std::string mat, bool player, std::string name,glm::v3 pos, glm::vec3 scl, Engine::Physics::Collision* collision,Scene* scene): ObjectDynamic(mesh,mat,pos,scl,name,collision,scene){
	m_WarpFactor = 0;
	m_IsPlayer = player;
	m_IsWarping = false;
	m_Target = nullptr;
	...
	for(unsigned int i = 0; i < SHIP_SYSTEM_NUMBER; i++){
		...
		if(i == 0)       system = new ShipSystemReactor(this,100);
...
void Ship::update(float dt){
	if(m_IsPlayer){
		...
		if(m_IsWarping && m_WarpFactor > 0){
			...
			glm::v3 s = (getForward() * glm::pow(speed,static_cast<glm::nType>(20.0L)))/static_cast<glm::nType>(getMass());
