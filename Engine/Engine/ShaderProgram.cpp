...
class ShaderP::impl{
	...
	public:
		void _construct(std::string vs, std::string ps, bool fromFile){
			m_VertexShader = vs;
			m_PixelShader = ps;
			m_FromFile = fromFile;
		}
		...
		GLuint _p(){return m_Shader;}
		std::string _vs(){return m_VertexShader;}
		std::string _ps(){return m_PixelShader;}
};

ShaderP::ShaderP(std::string vs, std::string ps, bool fromFile):m_i(new impl()){
	m_i->_construct(vs,ps,fromFile);
}
ShaderP::~ShaderP(){
    m_i->_destruct();
}
...
GLuint ShaderP::program(){ return m_i->_p(); }
std::string ShaderP::vertexShader(){ return m_i->_vs(); }
std::string ShaderP::pixelShader(){ return m_i->_ps(); }