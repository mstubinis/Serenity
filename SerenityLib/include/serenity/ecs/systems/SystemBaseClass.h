...
class SystemCRTP : public SystemBaseClass {
    ...
    private:
        ...
        COMPONENT* buildTupleImpl(Entity entity, uint32_t& idx) {
            COMPONENT* ret = getComponentPool<COMPONENT>(idx).getComponent(entity);
            ++idx;
            return ret;
        }

        template<class T>
        std::tuple<T, Entity, COMPONENTS*...> buildTuple(T args, Entity entity) {
            uint32_t idx = 0;
            return std::make_tuple(args, entity, buildTupleImpl<COMPONENTS>(entity, idx)...);
        }
        template<class T>
        std::tuple<T, COMPONENTS*...> buildTupleNoEntity(T args, Entity entity) {
            uint32_t idx = 0;
            return std::make_tuple(args, buildTupleImpl<COMPONENTS>(entity, idx)...);
        }
    ...
    public:
        ...
        void forEach(void(*func)(T, COMPONENTS*...), T args, SystemExecutionPolicy execPolicy = SystemExecutionPolicy::Normal) {
            using F = std::tuple<T, COMPONENTS*...>(SystemCRTP::*)(T, Entity);
            forEachImpl<T, decltype(func), F>(args, func, &SystemCRTP::buildTupleNoEntity, execPolicy);
        }
};
