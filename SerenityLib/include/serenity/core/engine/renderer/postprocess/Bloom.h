...
namespace Engine::Renderer::bloom {
    ...
    inline void setBlurRadius(float blurRadius) noexcept { Engine::priv::Bloom::bloom.m_Blur_Radius = std::max(0.0f, blurRadius); }
    inline void setBlurStrength(float blurStrength) noexcept { Engine::priv::Bloom::bloom.m_Blur_Strength = std::max(0.0f, blurStrength); }
    inline float getScale() noexcept { return Engine::priv::Bloom::bloom.m_Scale; }
    inline void setScale(float scale) noexcept { Engine::priv::Bloom::bloom.m_Scale = std::max(0.0f, scale); }
};
#endif