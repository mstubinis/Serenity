...
namespace Engine::priv {
    class AnimationData final {
        ...
        private:
            ...
            [[nodiscard]] glm::vec3 internal_interpolate_vec3(float AnimationTime, const std::vector<Engine::priv::Vector3Key>& keys, std::function<size_t()>&& call);

            template<typename T> [[nodiscard]] size_t internal_find(float AnimationTime, const AnimationChannel& node, const std::vector<T>& keys) const {
            ...
            [[nodiscard]] glm::vec3 CalcInterpolatedPosition(float AnimationTime, const AnimationChannel& node);
            [[nodiscard]] aiQuaternion CalcInterpolatedRotation(float AnimationTime, const AnimationChannel& node);
            [[nodiscard]] glm::vec3 CalcInterpolatedScaling(float AnimationTime, const AnimationChannel& node);
            [[nodiscard]] size_t FindPosition(float AnimationTime, const AnimationChannel& node) const;
            [[nodiscard]] size_t FindRotation(float AnimationTime, const AnimationChannel& node) const;
            [[nodiscard]] size_t FindScaling(float AnimationTime, const AnimationChannel& node) const;
