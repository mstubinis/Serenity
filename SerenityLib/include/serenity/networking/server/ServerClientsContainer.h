...
namespace Engine::Networking {
    class ServerClientsContainer final {
        ...
        public:
            ...
            [[nodiscard]] inline ServerClient* getClient(const std::string& hash) const noexcept { return (m_HashedClients.contains(hash)) ? m_HashedClients.at(hash) : nullptr; }
            [[nodiscard]] bool contains(const std::string& hash) const noexcept { return m_HashedClients.contains(hash); }
