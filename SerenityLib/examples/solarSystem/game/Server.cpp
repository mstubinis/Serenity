...
ServerClient::ServerClient(Server& server, Core& core, sf::TcpSocket* sfTCPSocket, const bool blocking) : m_Core(core), m_Server(server) {
    m_TcpSocket = new Networking::SocketTCP(sfTCPSocket);
    internalInit(blocking);
}
ServerClient::ServerClient(Server& server, Core& core, const ushort& port, const string& ipAddress, const bool blocking) : m_Core(core), m_Server(server) {
...
void ServerClient::internalInit(const bool blocking) {
    m_RecoveryTime = 0.0;
    m_Timeout      = 0.0;
    m_username     = "";
    m_Validated    = false;
    ...
    m_Active       = true;
    if (blocking) {
        auto updateClient = [&](ServerClient& client) {
            while (client.m_Active == true) {
                const auto active = client.m_Server.m_Active.load(std::memory_order_relaxed);
                if (active == 0)
                    return true;
        ...
        m_Thread = new std::future<bool>(std::move(std::async(std::launch::async, updateClient, std::ref(*this))));
    }
}
...
const bool Server::startup(const string& mapname) {
    ...
    if (status == sf::Socket::Status::Done) {
        ...
        new Map(m_MapName, ResourceManifest::BasePath + "data/Systems/" + m_MapName + ".txt");
        return true;
    }
    return false;
}
...
void Server::update(Server* thisServer, const double& dt) {
    ...
    const auto active = server.m_Active.load(std::memory_order_relaxed);
    if (active == 1) {
        server.m_DeepspaceAnchorTimer += dt;
        if (server.m_blocking) { //blocking (threads)
            updateAcceptNewClients(server);
            server.updateClientsGameLoop(dt);
            updateRemoveDisconnectedClients(server); //this doesn't really need it's own thread i think...
        }else{ //non-blocking (game update loop)
            updateAcceptNewClients(server);
    ...
    if (server.m_listener) {
        ...
        if (status == sf::Socket::Status::Done) {
            ...
            if (!server.m_clients.count(client_address)) {
                ...
                ServerClient* client = new ServerClient(server, server.m_Core, sf_client, server.m_blocking);
