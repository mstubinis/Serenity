...
struct BorgBeamCollisionFunctor final { void operator()(CollisionCallbackEventData& data) const {
    auto disruptorShipVoid = data.ownerBody.getUserPointer1();
    auto otherShipVoid = data.otherBody.getUserPointer1();
    if (otherShipVoid && disruptorShipVoid) {
        if (otherShipVoid != disruptorShipVoid) {//dont hit ourselves!
            Ship* otherShip = static_cast<Ship*>(otherShipVoid);
            if (otherShip) {
                auto& weapon = *static_cast<BorgBeam*>(data.ownerBody.getUserPointer2());
                auto& source = static_cast<Ship*>(disruptorShipVoid)->getMapKey();
...
struct BorgBeamInstanceBindFunctor { void operator()(EngineResource* r) const {
    //glDepthMask(GL_TRUE);
    auto& i = *static_cast<ModelInstance*>(r);
    Entity& parent = i.parent();
    auto& body = *parent.getComponent<ComponentBody>();
    auto& beam = *static_cast<BorgBeam*>(i.getUserPointer());
...
BorgBeam::BorgBeam(Ship& ship, Map& map, const glm_vec3& position, const glm_vec3& forward, const float& arc, vector<glm::vec3>& windupPts, const float& damage, const float& _chargeTimerSpeed, const float& _firingTime, const float& _impactRadius, const float& _impactTime, const float& _volume, const uint& _maxCharges, const float& _rechargeTimePerRound, const unsigned int& _modelIndex, const float& endpointExtraScale, const float& beamSizeExtraScale, const float& RangeInKM, const float& BeamLaunchSpeed) : PrimaryWeaponBeam(WeaponType::BorgBeam, ship, map, position, forward, arc, damage, _impactRadius, _impactTime, _volume, windupPts, _maxCharges, _rechargeTimePerRound, _chargeTimerSpeed, _firingTime, _modelIndex, endpointExtraScale, beamSizeExtraScale, RangeInKM, BeamLaunchSpeed) {
    ...
    beamModelInstance.setCustomBindFunctor(BorgBeamInstanceBindFunctor());
    beamModelInstance.setCustomUnbindFunctor(BorgBeamInstanceUnbindFunctor());
    ...
    beamEndBody.setCollisionFunctor(BorgBeamCollisionFunctor());
}
BorgBeam::~BorgBeam() {
...
const bool BorgBeam::fire(const double& dt, const glm_vec3& chosen_target_pt) {
...
const bool BorgBeam::forceFire(const double& dt) {
    if (state == BeamWeaponState::Off) {
        ...
        soundEffect = Engine::Sound::playEffect(ResourceManifest::SoundBorgBeam);
        if (soundEffect) {
...
void BorgBeam::update(const double& dt) {
