...
KlingonPhotonTorpedoProjectile::KlingonPhotonTorpedoProjectile(EntityWrapper* target, KlingonPhotonTorpedo& source, Map& map, const glm_vec3& final_world_position, const glm_vec3& forward, const int index, const glm_vec3& chosen_target_pos) :torpedo(source), SecondaryWeaponTorpedoProjectile(map, final_world_position, forward, index) {
    ...
    for (uint i = 0; i < 6; ++i) {
        ...
        KlingonPhotonTorpedoFlare flareD = KlingonPhotonTorpedoFlare(spin);
...
void KlingonPhotonTorpedoProjectile::update(const double& dt) {
    if (active) {
...
KlingonPhotonTorpedo::KlingonPhotonTorpedo(Ship& ship, Map& map, const glm_vec3& position, const glm_vec3& forward, const float& arc, const uint& _maxCharges, const float& _damage, const float& _rechargePerRound, const float& _impactRadius, const float& _impactTime, const float& _travelSpeed, const float& _volume, const float& _rotAngleSpeed, const unsigned int& _modelIndex) :SecondaryWeaponTorpedo(map, WeaponType::KlingonPhotonTorpedo, ship, position, forward, arc, _maxCharges, _damage, _rechargePerRound, _impactRadius, _impactTime, _travelSpeed, _volume, _rotAngleSpeed, _modelIndex) {

}
KlingonPhotonTorpedo::~KlingonPhotonTorpedo() {

}

void KlingonPhotonTorpedo::update(const double& dt) {
