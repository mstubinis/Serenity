...
struct BorgTorpedoCollisionFunctor final { void operator()(CollisionCallbackEventData& data) const {
    auto torpedoShipVoid = data.ownerBody.getUserPointer1();
    auto& torpedoProjectile = *static_cast<BorgTorpedoProjectile*>(data.ownerBody.getUserPointer());
        ...
        if (otherPtrShip != torpedoShipVoid) {//dont hit ourselves!
            ...
            if (otherShip && torpedoProjectile.active) {
                ...
                if (sourceShip->IsPlayer() || (sourceAI != AIType::AI_None && sourceAI != AIType::Player_Other)) {
                    auto& torpedo = *static_cast<BorgTorpedo*>(data.ownerBody.getUserPointer2());
...
struct BorgTorpedoInstanceCoreBindFunctor final { void operator()(EngineResource* r) const {
    //glDepthMask(GL_TRUE);
...
struct BorgTorpedoInstanceCoreUnbindFunctor final { void operator()(EngineResource* r) const {
    //glDepthMask(GL_FALSE);
}};

struct BorgTorpedoInstanceGlowBindFunctor final { void operator()(EngineResource* r) const {
    //glDepthMask(GL_TRUE);
    auto& i = *static_cast<ModelInstance*>(r);
    Entity& parent = i.parent();
    auto& body = *parent.getComponent<ComponentBody>();
...
struct BorgTorpedoInstanceGlowUnbindFunctor final { void operator()(EngineResource* r) const {
    //glDepthMask(GL_FALSE);
}};

struct BorgTorpedoFlareInstanceBindFunctor final { void operator()(EngineResource* r) const {
    //glDepthMask(GL_TRUE);
    auto& i = *static_cast<ModelInstance*>(r);
    Entity& parent = i.parent();
    auto& body = *parent.getComponent<ComponentBody>();
...
struct BorgTorpedoFlareInstanceUnbindFunctor final { void operator()(EngineResource* r) const {
    //glDepthMask(GL_FALSE);
}};



BorgTorpedoProjectile::BorgTorpedoProjectile(EntityWrapper* target, BorgTorpedo& source, Map& map, const glm_vec3& final_world_position, const glm_vec3& forward, const int index, const glm_vec3& chosen_target_pos) :torpedo(source), SecondaryWeaponTorpedoProjectile(map, final_world_position, forward, index) {
    maxTime = 30.5f;
    rotationAngleSpeed = source.rotationAngleSpeed;

    const auto borgGreen = glm::vec4(0.0f, 1.0f, 0.0f, 1.0f);
    const auto borgDarkerGreen = glm::vec4(0.083f, 0.77f, 0.083f, 1.0f);
    ...
    core.setCustomBindFunctor(BorgTorpedoInstanceCoreBindFunctor());
    core.setCustomUnbindFunctor(BorgTorpedoInstanceCoreUnbindFunctor());
    core.setColor(1.0f, 1.0f, 1.0f, 1.0f);
    core.setScale(0.5f);

    glow.setColor(borgGreen);
    glow.setScale(5.6f);
    glow.setCustomBindFunctor(BorgTorpedoInstanceGlowBindFunctor());
    glow.setCustomUnbindFunctor(BorgTorpedoInstanceGlowUnbindFunctor());
    ...
    for (uint i = 0; i < 14; ++i) {
        ...
        flare.setColor(borgDarkerGreen);
        ...
        BorgTorpedoFlare flareD = BorgTorpedoFlare(spin);
    ...
    body.setCollisionFunctor(BorgTorpedoCollisionFunctor());
...
BorgTorpedoProjectile::~BorgTorpedoProjectile() {
}
void BorgTorpedoProjectile::update(const double& dt) {
    if (active) {
...
BorgTorpedo::BorgTorpedo(Ship& ship, Map& map, const glm_vec3& position, const glm_vec3& forward, const float& arc, const uint& _maxCharges, const float& _damage, const float& _rechargePerRound, const float& _impactRadius, const float& _impactTime, const float& _travelSpeed, const float& _volume, const float& _rotAngleSpeed, const unsigned int& _modelIndex) :SecondaryWeaponTorpedo(map, WeaponType::BorgTorpedo, ship, position, forward, arc, _maxCharges, _damage, _rechargePerRound, _impactRadius, _impactTime, _travelSpeed, _volume, _rotAngleSpeed, _modelIndex) {
...
void BorgTorpedo::update(const double& dt) {
