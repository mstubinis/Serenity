...
Sovereign::Sovereign(Scene& scene, glm::vec3 position, glm::vec3 scale)
...
:Ship(team, client, CLASS, map, ai_type, name, position, scale, collisionType, glm::vec3(0.0f, 0.0f, -1.58693f)) {
    ...
    m_Perks         = Sovereign::Perks::None;
    m_UnlockedPerks = Sovereign::Perks::None;

    auto& _this = *this;
    for (uint i = 0; i < ShipSystemType::_TOTAL; ++i) {
        ShipSystem* system = nullptr;
        if (i == 0)        system = NEW ShipSystemReactor(_this, 1000);
        else if (i == 1)   system = NEW ShipSystemPitchThrusters(_this, 0.27f);
        else if (i == 2)   system = NEW ShipSystemYawThrusters(_this, 0.27f);
        else if (i == 3)   system = NEW ShipSystemRollThrusters(_this, 0.19f);
            ...
            150500.0f,
            200500.0f,
            200500.0f, glm::vec3(0.0f), glm::vec3(1.05f, 1.25f, 1.05f), (m_UnlockedPerks & Sovereign::Perks::RegenerativeShields) ? 3000.0f : 1500.0f
        );
        ...
        else if (i == 10)  system = NEW ShipSystemHull(_this, map, 175500.0f, 285.0f, 5.0f);
