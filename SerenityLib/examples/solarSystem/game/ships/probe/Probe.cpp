...
constexpr auto CLASS = "Probe";

using namespace std;

Probe::Probe(Scene& scene, glm::vec3 position, glm::vec3 scale)
    :Ship(CLASS, scene, position, scale) {

}

Probe::Probe(AIType::Type& ai_type, Team& team, Client& client, Map& map, const string& name, glm::vec3 position, glm::vec3 scale, CollisionType::Type collisionType)
:Ship(team, client, CLASS, map, ai_type, name, position, scale, collisionType) {
    ...
    for (uint i = 0; i < ShipSystemType::_TOTAL; ++i) {
        ...
        else if (i == 10)  system = NEW ShipSystemHull(_this, map, 35500.0f);
