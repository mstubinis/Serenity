...
struct Host_ButtonNext_OnClick { void operator()(Button* button) const {
    ...
    const string& map        = hostScreen.getMapSelectionWindow().getCurrentChoice();
...
HostScreen::HostScreen(Menu& menu, Font& font) : m_Menu(menu){
    ...
    const auto contentSize = winSize - glm::uvec2(padding_x, padding_y);
    ...
    m_ServerHostMapSelector = NEW ServerHostingMapSelectorWindow(font, padding_x, winSize.y - padding_y, static_cast<float>(contentSize.x) / 3.0f, static_cast<float>(contentSize.y) / 2.0f);

    m_BackButton = NEW Button(font, 100.0f, 50.0f, 150.0f, 50.0f);
    m_BackButton->setText("Back");
    m_BackButton->setColor(0.5f, 0.5f, 0.5f, 1.0f);
    m_BackButton->setTextColor(1.0f, 1.0f, 0.0f, 1.0f);

    m_ForwardButton = NEW Button(font, winSize.x - 100.0f, 50.0f, 150.0f, 50.0f);
...
void HostScreen::onResize(const unsigned int newWidth, const unsigned int newHeight) {
    ...
    m_ForwardButton->setPosition(winSize.x - 100.0f, 50.0f);

    m_ServerHostMapSelector->setPosition(padding_x, winSize.y - padding_y);
