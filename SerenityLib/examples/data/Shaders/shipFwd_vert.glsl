...
void main(){
    ...
    vec3 NormalTrans   =  (BoneTransform * vec4(normal.zyx,   0.0)).xyz;  //Order is ZYXW so to bring it to XYZ we need to use ZYX
    vec3 BinormalTrans =  (BoneTransform * vec4(binormal.zyx, 0.0)).xyz;  //Order is ZYXW so to bring it to XYZ we need to use ZYX
    vec3 TangentTrans  =  (BoneTransform * vec4(tangent.zyx,  0.0)).xyz;  //Order is ZYXW so to bring it to XYZ we need to use ZYX
