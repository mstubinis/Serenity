...
struct GameCameraLogicFunctor final { void operator()(ComponentLogic2& _component, const double& dt) const {
    ...
    switch (camera.m_State) {
        case CameraState::Follow: {
			...
			const EntityDataRequest dataRequest1(targetEntity);
			...
			const glm::vec3 pos = targetBody.position() + ((targetBody.forward() * glm::length(targetRadius) * 1.7f) + targetBody.up() * glm::length(targetRadius) * 0.3f) * (1.0f + camera.m_OrbitRadius);
        ...
        case CameraState::FollowTarget: {
			...
			const EntityDataRequest dataRequest1(targetEntity);
			const EntityDataRequest dataRequest2(playerEntity);
			...
			const glm::vec3 pos(model[3][0], model[3][1], model[3][2]);
