...
void VertexDataFormat::add(int numComponents, int type, bool normalized, int stride, size_t offset, size_t typeSize) {
...
void VertexDataFormat::bind(const VertexData& vertData) const noexcept {
    if (m_InterleavingType == VertexAttributeLayout::Interleaved) {
        ...
        for (GLuint i = 0; i < m_Attributes.size(); ++i) {
            ...
            accumulator += vertData.m_Data[i].getSize() * attribute.typeSize;
        }
    }
}
