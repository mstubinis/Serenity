    ...
    if (ShaderHelper::sfind(code, "SpecularGGX(")) {
        if (!ShaderHelper::sfind(code, "vec3 SpecularGGX(")) {
            const string specular_ggx =
                "vec3 SpecularGGX(inout vec3 _Frensel,vec3 _LightDir,vec3 _Half,float _alpha,float _NdotH,vec3 _F0,float _NdotL){//generated\n"
    ...
    if (ShaderHelper::sfind(code, "SpecularCookTorrance(")) {
        if (!ShaderHelper::sfind(code, "vec3 SpecularCookTorrance(")) {
            const string specular_cook_torrance =
                "vec3 SpecularCookTorrance(inout vec3 _Frensel,vec3 _F0,float _VdotH,float _NdotH,float _alpha,float _VdotN,float _roughness,float _NdotL){//generated\n"
