...
namespace Engine::Renderer::ssao {
    ...
    inline void setBlurRadius(float blurRadius) noexcept { Engine::priv::SSAO::STATIC_SSAO.m_ssao_blur_radius = glm::max(0.0f, blurRadius); }
    inline float getBlurStrength() noexcept { return Engine::priv::SSAO::STATIC_SSAO.m_ssao_blur_strength; }
    inline void setBlurStrength(float blurStrength) noexcept { Engine::priv::SSAO::STATIC_SSAO.m_ssao_blur_strength = glm::max(0.0f, blurStrength); }
    inline float getIntensity() noexcept { return Engine::priv::SSAO::STATIC_SSAO.m_ssao_intensity; }
    inline void setIntensity(float intensity) noexcept { Engine::priv::SSAO::STATIC_SSAO.m_ssao_intensity = glm::max(0.0f, intensity); }
    inline float getRadius() noexcept { return Engine::priv::SSAO::STATIC_SSAO.m_ssao_radius; }
    inline void setRadius(float radius) noexcept { Engine::priv::SSAO::STATIC_SSAO.m_ssao_radius = glm::max(0.0f, radius); }
    inline float getScale() noexcept { return Engine::priv::SSAO::STATIC_SSAO.m_ssao_scale; }
    inline void setScale(float scale) noexcept { Engine::priv::SSAO::STATIC_SSAO.m_ssao_scale = glm::max(0.0f, scale); }
    ...
    inline void setSamples(uint32_t numSamples) noexcept { Engine::priv::SSAO::STATIC_SSAO.m_ssao_samples = glm::clamp(glm::max(0U, numSamples), 0U, SSAO_MAX_KERNEL_SIZE); }
};
#endif