    ...
    namespace priv{
        class EngineCore final {
            ...
            private:
                ...
                void on_event_resize(Window& window, unsigned int w, unsigned int h, bool saveSize);
                ...
                void on_event_window_closed(Window& window);
                ...
                void on_event_window_requested_closed(Window& window);
                ...
                void on_event_lost_focus(Window& window);

                /* The sf::Event::LostFocus and sf::Event::GainedFocus events are triggered when the window loses/gains focus,
                which happens when the user switches the currently active window. When the window is out of focus, it doesn't receive keyboard events. */
                void on_event_gained_focus(Window& window);
                ...
                void on_event_text_entered(Window& window, unsigned int unicode);
                ...
                void on_event_key_pressed(Window& window, unsigned int key);
                ...
                This event is the one to use if you want to trigger an action exactly once when a key is pressed or released,
                like making a character jump with space, or exiting something with escape. */
                void on_event_key_released(Window& window, unsigned int key);
                ...
                void on_event_mouse_wheel_scrolled(Window& window, float delta, int mouseWheelX, int mouseWheelY);
                ...
                void on_event_mouse_button_pressed(Window& window, unsigned int mouseButton);

                /* The sf::Event::MouseButtonPressed and sf::Event::MouseButtonReleased events are
                triggered when a mouse button is pressed/released.
                SFML supports 5 mouse buttons: left, right, middle (wheel), extra #1 and extra #2 (side buttons). */
                void on_event_mouse_button_released(Window& window, unsigned int mouseButton);
                ...
                void on_event_mouse_moved(Window& window, int mouseX, int mouseY);

                /* The sf::Event::MouseEntered and sf::Event::MouseLeft events are triggered when the mouse cursor enters/leaves the window. */
                void on_event_mouse_entered(Window& window);

                /* The sf::Event::MouseEntered and sf::Event::MouseLeft events are triggered when the mouse cursor enters/leaves the window. */
                void on_event_mouse_left(Window& window);


                void on_event_joystick_button_pressed(Window& window, unsigned int button, unsigned int id);
                void on_event_joystick_button_released(Window& window, unsigned int button, unsigned int id);
                void on_event_joystick_moved(Window& window, unsigned int id, float position, unsigned int axis);
                void on_event_joystick_connected(Window& window, unsigned int id);
                void on_event_joystick_disconnected(Window& window, unsigned int id);

                void update_logic(Scene& scene, Window& window, const float dt);
                void update_sounds(Scene& scene, Window& window, const float dt);
                void update_physics(Scene& scene, Window& window, const float dt);
